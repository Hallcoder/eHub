...
function Search({hide}) {
  const navigate = useNavigate();
    ...
    const adminIndex = algoliasearch(import.meta.env.VITE_ALGOLIA_APPID,import.meta.env.VITE_ALGOLIA_ADMINKEY).initIndex('products');
    ...
    return ( 
        <div className='sm:h-[75vh] sm:w-9/12  border-blue-500 p-1 rounded-md bg-white absolute border bg-opacity-90'>
            <MdClose className='w-1/12 m-2 h-10 border rounded-full float-right' onClick={() => hide()}/>
            <InstantSearch 
